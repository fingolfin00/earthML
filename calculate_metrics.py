...
from earthml.metrics import Metrics, plot_map_metric_grid #, PowerSpectrum
...
if __name__ == "__main__":
    ...
    vars_weather = {
        ...
        # 't2m_het': {'exp_var': 't2m', 'region': 'ConUS', 'exp_suffix': '_32bs_juno_heteroloss', 'train_period': train_period, 'test_period': test_period},
    }
    ...
    runs_weather = add_ke_to_runs(runs_weather, suffixes=("_mse",))

    metrics = {}
    for name, run in runs_weather.items():
        print(f"Run {name}")
        m = Metrics(truth=run['an'], data=[run['fc'], run['pr']], truth_name="ECMWF an", data_name=["ECMWF fc", "MLFC pr"])
    ...
    for var_name in vars_weather_from_fc:
        save_path = Path(exp_root_folder_weather) / Path("plots") / Path(
            f"{experiments_weather['name']}_{var_name}_{experiments_weather['test_period']}" \
