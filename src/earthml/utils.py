...
# Pretty rich table generation helper class
class Table:
...
def inpaint_nan_bilinear (ds: xr.Dataset) -> xr.Dataset:
    ...
    return (
        ds
        .interpolate_na(dim=guess_lat_dim(ds), method="linear", use_coordinate=False)
...
def normalize_ds_dims_and_coords (ds: xr.Dataset) -> xr.Dataset:
    ...
    """
    for guessed_dim in (
        (guess_time_dim(ds), "time"),
        (guess_lat_dim(ds), "latitude"),
        (guess_lon_dim(ds), "longitude"),
        (guess_realization_dim(ds), "realization"),
        (guess_leadtime_dim(ds), "leadtime")
    ):
