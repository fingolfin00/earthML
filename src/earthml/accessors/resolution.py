...
XarrayObject = Union[xr.Dataset, xr.DataArray]
...
class EarthMLResolution:
    ...
    def _coord_resolution_1d(
        ...
        *,
        ...
        """
        Infer 1D coordinate resolution in degrees.

        For longitude, unwrap values first so dateline crossings do not produce
        artificial ~360 degree jumps.
        """
        ...
        if is_lon:
            vals_rad = np.deg2rad(vals)
            vals = np.rad2deg(np.unwrap(vals_rad))
    ...
    @staticmethod
    def _spacing_stats_line(
        line_deg: npt.ArrayLike,
        *,
        is_lon: bool = False,
    ...
    def _is_regular_curvilinear(
        ...
        *,
        ...
        lat_mean, _, lat_cv = self._spacing_stats_line(lat_line, is_lon=False)
        lon_mean, _, lon_cv = self._spacing_stats_line(lon_line, is_lon=True)

        n_lat = int(np.isfinite(lat_line).sum())
        n_lon = int(np.isfinite(lon_line).sum())

        if (
            np.isnan(lat_mean)
            or np.isnan(lon_mean)
            or n_lat < min_points
            or n_lon < min_points
        ):
    ...
    def resolution(self) -> tuple[float, float]:
        ...
        if lat_da.ndim == 1 and lon_da.ndim == 1:
            lat_res = self._coord_resolution_1d(
                lat_da.values,
                lat_coord,
                is_lon=False,
            )
            lon_res = self._coord_resolution_1d(
                lon_da.values,
                lon_coord,
                is_lon=True,
            )
        ...
        if lat_da.ndim == 2 and lon_da.ndim == 2:
            ...
            is_reg, (lat_reg, lon_reg) = self._is_regular_curvilinear(
                lat2d,
                lon2d,
                rel_tol=0.5,
            )
            ...
            logger.info(
                "Grid appears non-regular; using fallback curvilinear heuristic."
            )
        ...
        logger.info("Mixed lat/lon dimensions; treating separately.")
        ...
        if lat_da.ndim == 1:
            ...
            lat_values = lat_da.values
            lat_line = lat_values[:, lat_values.shape[1] // 2]

            lat_mean, _, _ = self._spacing_stats_line(
                lat_line,
                is_lon=False,
            )
        ...
        if lon_da.ndim == 1:
            ...
            lon_values = lon_da.values
            lon_line = lon_values[lon_values.shape[0] // 2, :]
